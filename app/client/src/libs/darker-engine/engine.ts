...
export const engine = <
	...
	D,
>(): EngineType<I, C, D> => {
	let rawSystems: SystemFunction<C>[] = [];
	...
	const setSystems = async (..._systems: SystemFunction<C>[]) => {
		...
		for await (let system of _systems) {
	...
	const _entity_removeComponent = async (entityId: number, component: C) => {
		...
		for await (let system of systemList) {
			const entities = systemEntitiesMap[system.id];
			systemEntitiesMap[system.id] =
				entities?.filter(_id => _id !== entityId) ?? [];
			try {
	...
	const _entity_getComponentTypes = (entityId: number) =>
	...
	const getEntityList = (): EntityType<I, C, D>[] =>
	...
	const addEntity = async (
		...
		const entities = rawEntities.map(rawEntity => {
			...
			entityDataMap[entity.id] =
				entity?.getComponentTypes?.().reduce(
					(acc, b) => ({
						...acc,
						[b]: (acc as any)[b] || {},
					}),
		...
		for await (let entity of entities) {
			...
			const componentMapPoint: { [key: string]: number } =
	...
	const removeEntity = async (...entityIdList: number[]) => {
		...
		for await (let entity of _entityList) {
			if (!entity) return;
	...
	const debug = () => {
		...
		const getSystem = (name: string): SystemType<C> =>
